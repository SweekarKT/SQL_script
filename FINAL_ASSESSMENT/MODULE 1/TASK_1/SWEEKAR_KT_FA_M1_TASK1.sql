...
TRUNCATE TABLE FA_RESOURCE_FROM_JOB_PORTAL;
...
--This view fetches the years of experience of the resource in a company

CREATE VIEW FA_M1_T1_RSRC_COMP_SEP_EXP AS 
SELECT RD.r_id,r_name,r_comp_name,r_comp_rol1_exp+(CASE 
	WHEN r_comp_ro2_exp IS NULL THEN 0
	ELSE r_comp_ro2_exp
END
) AS TOT_COMP_EXP FROM FA_RSRC_DETAILS AS RD
INNER JOIN FA_RS_COMP_DETAILS AS CD ON RD.r_id = CD.r_id;

SELECT * FROM FA_M1_T1_RSRC_COMP_SEP_EXP;
DROP VIEW FA_M1_T1_RSRC_COMP_SEP_EXP;


--This view fetches the number of companies worked in and total years of experience of the resource
CREATE VIEW FA_M1_T1_RSRC_TOTAL_EXP AS
SELECT r_id,MIN(r_name) AS name,COUNT(r_name) AS no_comp_worked,SUM(TOT_COMP_EXP) AS TOTAL_EXP FROM FA_M1_T1_RSRC_COMP_SEP_EXP
GROUP BY r_id;

SELECT * FROM FA_M1_T1_RSRC_TOTAL_EXP;