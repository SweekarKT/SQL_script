...
--Calculating the total average percentage of each customer feedback
SELECT DISTINCT ((SELECT COUNT(CustomerFeedback) FROM FA_M2_SCD2_CAR_SALES WHERE CustomerFeedback LIKE 'Very Bad')+.1)/
(SELECT COUNT(CustomerFeedback) FROM FA_M2_SCD2_CAR_SALES)*100 AS Very_Bad,
((SELECT COUNT(CustomerFeedback) FROM FA_M2_SCD2_CAR_SALES WHERE CustomerFeedback LIKE 'Bad')+.0)/
(SELECT COUNT(CustomerFeedback) FROM FA_M2_SCD2_CAR_SALES)*100 AS Bad,
((SELECT COUNT(CustomerFeedback) FROM FA_M2_SCD2_CAR_SALES WHERE CustomerFeedback LIKE 'Okay')+.0)/
(SELECT COUNT(CustomerFeedback) FROM FA_M2_SCD2_CAR_SALES)*100 AS Okay,
((SELECT COUNT(CustomerFeedback) FROM FA_M2_SCD2_CAR_SALES WHERE CustomerFeedback LIKE 'Bad')+.0)/
(SELECT COUNT(CustomerFeedback) FROM FA_M2_SCD2_CAR_SALES)*100 AS Bad,
((SELECT COUNT(CustomerFeedback) FROM FA_M2_SCD2_CAR_SALES WHERE CustomerFeedback LIKE 'Very Bad')+.0)/
(SELECT COUNT(CustomerFeedback) FROM FA_M2_SCD2_CAR_SALES)*100 AS Very_bad
FROM FA_M2_SCD2_CAR_SALES;

SELECT (1+.0)/2 FROM FA_DS1_LOAN_APPROVAL;